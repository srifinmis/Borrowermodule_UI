...
const LenderMasterApproval = ({ isDropped }) => {
    ...
    useEffect(() => {
        const fetchLenders = async () => {
            ...
            try {
                const response = await axios.get(`${API_URL}/roc/pendingData`);
    ...
    const handleApprove = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${API_URL}/roc/Approve`, selectedRows.map((lender) => ({
                ...lender,
            ...
            if (response.status === 201) {
                ...
                localStorage.setItem("submissionMessage", "ROC Forms Approval failed!");
    ...
    const handleReject = async () => {
        ...
        try {
            const response = await axios.post(`${API_URL}/roc/reject`,
            ...
            if (response.status === 201) {
                message.success("ROC Forms rejected successfully.");
    ...
    const handleViewDetails = (sanction_id, lender_code, approval_status, updatedat) => {
        navigate(`/rocapprove/${sanction_id}`, {
